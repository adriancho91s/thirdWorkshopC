...
double taylorSen(int x, int enesimo) {
    if (enesimo == 0) {
        return pow(x, 2 * enesimo + 1) / factorial(2 * enesimo + 1) * pow(- 1, enesimo);
    } else {
        return pow(x, 2 * enesimo + 1) / factorial(2 * enesimo + 1) * pow(- 1, enesimo) + taylorSen(x, enesimo - 1);
    }
}
