...
double factorial(double numero, double acumulador) {
    return (numero == 0) ? acumulador : factorial(numero - 1, acumulador * numero);
}// Esta función calcula el factorial de un número n.

double catalan(double enesimo) {
    return ( factorial(2 * enesimo, 1) / (  factorial(enesimo, 1) * (factorial(enesimo + 1, 1))  ) );
}// Esta función calcula el número de Catalan de n.

void imprimirCatalan(int enesimo){
    enesimo == 0 ? printf("%.0f", catalan(enesimo)) : 
    (imprimirCatalan(enesimo - 1), printf(", %.0f", catalan(enesimo)));
